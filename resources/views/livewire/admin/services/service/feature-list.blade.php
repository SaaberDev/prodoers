<div>
    @forelse($service->serviceFeatures as $key => $serviceFeatures)
        <div id="dynamic-field-single-{{ $key + 1 }}" class="dynamic-field-single mb-3" wire:key="{{ $loop->index }}">
            <label class="mr-3 mt-2" for="field-{{ $loop->iteration }}">Features<span class="ctm-required">*</span></label>
            <div class="input-group-append">
                <input type="text" id="field-{{ $loop->iteration }}" value="{{ $serviceFeatures->feature_desc }}" class="form-control validation-single" name="features[]" aria-describedby="button-addon2">
